...
if [[ -z "$spec_text" ]]; then
  cat <<'EOF'
Usage: /mygoal start "<spec text>" [max_turns] [max_tokens]
...
Examples:
  /mygoal start "Implement a /mygoal slash command per design doc /tmp/goal-design.md. Done means: SKILL.md exists, all scripts pass shellcheck, README documents the kill switch."
EOF
  exit 1
fi
...
Options:
  /mygoal abort        # terminate the current goal first
  /mygoal show-spec    # see what's already running
  /mygoal status       # full state
EOF
    exit 1
  fi
fi
...
Goal initialized.
  ...
  kill:       /mygoal abort   OR   touch $(goal_stop_path)
