...
**Status**: v1.1 — round-2 audit 后修订:故宫/国博 MAU 降级为明确猜测(A9/A10),小红书数据换真引用(B3),维度数 5→7+1,moat 循环论证降级
**Word count**: ~2500
...
- **量级**: **精确 App MAU 无公开数据** [A·A9]——原文写的"~150k(2024 行业报告)"是我猜的,无此报告。**有据可查的是** [B4]:故宫 95万件文物数字化、10万件高清公开、"畅游多宝阁"单周 500万人次、数字故宫小程序用户 >70% 在 40岁以下(文旅部/腾讯 2024)
...
- **我们的差异化**:
  ...
  2. **Knowledge graph** — 识别完之后给 7+1 维度结构化信息(形制/时代/地理/纹饰/铭文/用途/铸主 + 稀有度),不是搜索结果
...
- **量级** [B3]: 小红书 MAU 3.5亿、18-34岁 >70%;单"广东省博物馆"话题累计浏览即 9134万;"博物馆"长居小红书出行热词 TOP10(数英/千瓜 2024)。(原文笼统写的"话题 100M+ / B站 ~30M/月"未核实,已替换为可引用数据)
...
| **MuseumCollect(我们)** | ★★★★ (青铜先深,其他 vertical 逐步) | ★★★ (CLIP+Phase 2 ensemble) | ★★★★ (7+1 维度 collection) | — |
...
| **产品设计 IP**(motivation hook × 维度结构) | ★★★?(自评,**未验证**) | **假设型 moat,不是已证明的** — round-2 audit 抓到我原来给 ★★★★★"强"、理由是"别人想不到",这是**用自己的信念证明强度,不可证伪**。诚实降级:设计 IP 是否是 moat,取决于 §4.3 的模仿者测试结果,现在打 ★★★★★ 是自我服务 |
| Museum partnership | ★★★★ | 中强 — 排他性 partnership 是**真**护城河(可验证:签没签下来),但 BD 难度大 |

### 4.2 主 moat 假设

> **设计 IP 是 V1-V2 的 *假设型* moat,museum partnership + user data 是 V3+ 的 *可验证* moat。**

⚠️ **round-2 audit 的诚实修正**: 我承认整个 defensibility 故事有个弱点——我自己说了算法不是 moat、数据不是 moat,那剩下能撑早期的就只有"设计 IP",而设计 IP 的强度**完全建立在我自己认为它非显然**之上,这是循环论证。真正能验证的 moat(partnership 签没签下、user data churn 低不低)都要时间才长出来。所以诚实的结论是:**早期我没有已证明的 moat,只有一个待验证的设计 IP 假设 + 速度**。这比"我有 ★★★★★ 的设计护城河"诚实。
